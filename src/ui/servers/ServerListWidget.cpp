...
std::map<QString, QByteArray> _serverToIconMap;
char selected_ip[64] = "";
char selected_port[16] = "";

typedef std::pair<QString, QString> BCPair;
Q_DECLARE_METATYPE(BCPair);
...
void ServerListWidget::onServerListFetchFinish() {
...
void ServerListWidget::onServerClicked(QListWidgetItem* item) {
    BCPair serverInfo = item->data(Qt::UserRole).value<BCPair>();
...
void ServerListWidget::addServerItem(bc_server server) {
    ...
    BCPair serverInfo =
            *(new BCPair(QString(server.connect_socket), QString(server.connect_version)));
