...
Q_DECLARE_METATYPE(bc_mod_version);
...
InstanceEditModsWidget::InstanceEditModsWidget(QWidget* parent)
    ...
    _modList->header()->resizeSections(QHeaderView::ResizeToContents);
...
bc_mod_version_array* InstanceEditModsWidget::getSettings() {
    ...
    for (int i = 0; i < order->len; i++) {
        ...
        bc_mod_version mod = _modList->model()->data(index, Qt::UserRole).value<bc_mod_version>();
        order->arr[i] = mod;
    }
