...
Q_DECLARE_METATYPE(bc_instance);
...
void InstanceListWidget::populateInstanceList() {
    ...
    QList<int> qlist = groups.toList();
    for (int in = 0; in < qlist.size(); in++) {
        _instanceList->topLevelItem(qlist.at(in))->setExpanded(1);
    }
}
...
void InstanceListWidget::moveInstanceList() {
    ...
    for (int i = 0; i < _instanceList->model()->rowCount(); i++) {
        ...
        // TODO: sus. changed from itemFromIndex(QModelIndex)
        QTreeWidgetItem* item = _instanceList->itemAt(index.column(), index.row());
