...
void MainWindow::onStartup() {
    if (betacraft_online) {
        QFuture<char*> future = QtConcurrent::run(bc_network_get, (char*)"https://raw.githubusercontent.com/betacraftuk/betacraft-launcher/v2/CHANGELOG.md", (char*)"");
        _changelogWatcher.setFuture(future);

        QFuture<int> updateFuture = QtConcurrent::run(bc_update_check, (char*)_updateVersion);
...
void MainWindow::initChangelog() {
    ...
    if (!betacraft_online) {
        _changelog->setText("Offline");
        return;
    }

    _changelog->setText("Loading...");
}
...
void MainWindow::loadChangelog() {
    ...
    _changelog->setText(QString(response));
    free(response);
}
...
void MainWindow::launchGameNoServer() {
    launchGame("", "");
}
...
void MainWindow::onToggleTabs() {
    ...
    QTabBar* tabbar = _menu->findChild<QTabBar*>(QLatin1String("qt_tabwidget_tabbar"));
    tabbar->setDisabled(tabbar->isEnabled());
}
...
void MainWindow::onInstanceUpdate() {
    ...
    if (instance != NULL) {
        ...
        _instanceSelectedVersion = QString();
        _instanceSelectedName = QString();
